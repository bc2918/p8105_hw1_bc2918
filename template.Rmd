...
Description: 
the `penguins` dataset includes the following variables: ``r names(penguins)``; it has ``r nrow(penguins)`` rows and ``r ncol(penguins)`` columns. The mean of the flipper length is `r mean(na.omit(pull(penguins, flipper_length_mm)))`.
